#!/bin/bash
#SBATCH --job-name=ccn_temporal_maxpool
#SBATCH --output=logs/temporal_maxpool_%A.out
#SBATCH --error=logs/temporal_maxpool_%A.out
...
OUTPUT_DIR=/srv/beegfs/scratch/shares/schaerm/schaer2/video_sam2_pose/humanLISBET-paper/baselineEval/results_stats_temporal_maxpool_lasso

# ── Stat configurations for standard representations ─────────
# Standard representations (motor_only, full_kinematic, embedding, combined) also
# run alongside temporal representations so results are directly comparable.
STATS_KIN="mean,std,kurtosis,skewness"
STATS_EMB="mean,std"

echo "Output: $OUTPUT_DIR"
echo "Standard stats: --stats-kin $STATS_KIN  --stats-emb $STATS_EMB"
echo "Temporal pooling: enabled (N_TEMPORAL_SEGMENTS=35)"

# ── Run ──────────────────────────────────────────────────────
python -m baselineEval.src.run_pipeline \
    --csv              "$CSV" \
...
echo "SLURM job $SLURM_JOB_ID finished with exit code $?"
