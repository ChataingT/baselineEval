#!/bin/bash
#SBATCH --job-name=ccn_eval
#SBATCH --output=logs/run_pipeline_%j.out
#SBATCH --error=logs/run_pipeline_%j.out
...
OUTPUT_DIR=/srv/beegfs/scratch/shares/schaerm/schaer2/video_sam2_pose/humanLISBET-paper/ccnEvaluation/results
...
python -m ccnEvaluation.src.run_pipeline \
    ...
    --use-gpu \
    --log-level      INFO
    # --debug-n 5    # uncomment for quick debugging
