...
#SBATCH --output=logs/sweep_stats_%A.out
#SBATCH --error=logs/sweep_stats_%A.out
...
OUTPUT_BASE=/srv/beegfs/scratch/shares/schaerm/schaer2/video_sam2_pose/humanLISBET-paper/baselineEval/results_stats

# ── Stat configurations ──────────────────────────────────────
STATS_KIN="mean,std,kurtosis,skewness"
STATS_EMB="mean,std"
OUTPUT_DIR="${OUTPUT_BASE}_extended_target_2"

echo "--stats-kin $STATS_KIN  --stats-emb $STATS_EMB → $OUTPUT_DIR"
